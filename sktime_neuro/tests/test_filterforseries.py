...
def test_bad_input_args(l_freq, h_freq):
    ...
    if not (
        ...
        with pytest.raises((TypeError)):
            FilterforSeries(l_freq=l_freq, h_freq=h_freq, sfreq=250)

    elif (l_freq is not None) & (h_freq is not None):
        if not ((l_freq > 0) & (h_freq > 0)):
            with pytest.raises(ValueError):
                Filter = FilterforSeries(l_freq=l_freq, h_freq=h_freq, sfreq=250)
...
def test_filter_kwargs(kwargs):
    np.random.seed(42)
    X = 0.02 * np.random.randn(30, 1000)
    filterforpanel = FilterforSeries(sfreq=250, l_freq=1, h_freq=14, **kwargs)
